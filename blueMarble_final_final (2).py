...
dic1={'출발':0,'타이베이':1,'베이징':2,'제주':3,'황금열쇠1':4,'카이로':5,'이스탄불':6,'무인도':7,'아테네':8,\
    '원주':9,'코펜하겐':10,'미래관':11,'베를린':12,'오타와':13,'세금받기':14,'창조관':15,'상파울로':16,\
    '부산':17,'리스본':18,'황금열쇠2':19,'마드리드':20,'우주여행':21,'도쿄':22,'파리':23,'황금열쇠3':24,'대학본부':25,\
    '서울':26,'국세청':27}

dic2={0:'출발' , 1: '타이베이', 2 : '베이징' , 3:'제주',4:'황금열쇠1', 5:'카이로',6: '이스탄불' , 7: '무인도',  8: '아테네' , \
    9: '원주' ,10: '코펜하겐' ,11:'미래관', 12: '베를린' , 13: '오타와' , 14: '세금받기' ,15 : '창조관' ,16:'상파울로' , \
    17: '부산' , 18: '리스본' , 19: '황금열쇠2' , 20: '마드리드' , 21: '우주여행' ,22: '도쿄' , 23: '파리' ,24: '황금열쇠3' , 25:'대학본부',\
    26: '서울' , 27:'국세청'}
...
def Charactor_Screen():
    ...
    while True:
        # clock.tick(10)
        """캐릭터 이름설명"""
        CharactorSelect((padWidth//8)*0.8,200,'투기자', WHITE, BLACK, 2)
        CharactorSelect(3*(padWidth//8)*0.87,200, '여행자',WHITE, BLACK,2)
        CharactorSelect(5*(padWidth//8)*0.88,200, '선행자',WHITE, BLACK,2)
        CharactorSelect(7*(padWidth//8)*0.97,200, '학고생',WHITE, BLACK,2)
        """캐릭터 능력, 승리조건, 배경이야기"""
        CharactorSelect((padWidth // 8) * 0.02, 400, '20년 전 무조건 재개발 된다는 친구의 말만 믿고', WHITE, BLACK) #투기자
        ...
        CharactorSelect((3 * (padWidth // 8)) * 0.68, 400, '삶속에서 행운이 가득넘쳐 사는게 참 쉽다 ', WHITE, BLACK) #여행자
        CharactorSelect((3 * (padWidth // 8)) * 0.68, 430, '자신의 운을 마블게임에서도 검증하고 싶어한다.', WHITE, BLACK)
        ...
        CharactorSelect((3 * (padWidth // 8)) * 0.68, 580, "대한민국의 도시 4곳 중 4곳을 얻으면 승리!", WHITE, BLACK)
        ...
        CharactorSelect((5 * (padWidth // 8)) * 0.81, 400, '이시대의 참인간', WHITE, BLACK)# 선행자
        CharactorSelect((5 * (padWidth // 8)) * 0.81, 430, '1년 365일 바른생활인간으로 살아온 ', WHITE, BLACK)
        CharactorSelect((5 * (padWidth // 8)) * 0.81, 460, '이 착한 남자의 유일한 소원은', WHITE, BLACK)
        CharactorSelect((5 * (padWidth // 8)) * 0.81, 490, '굳게 존재할 것이라 믿는 신과의 만남이다.', WHITE, BLACK)
        ...
        CharactorSelect((7 * (padWidth // 8)) * 0.867, 400, '20살 어린나이에 대학에 들어와', WHITE, BLACK) # 학고생
        CharactorSelect((7 * (padWidth // 8)) * 0.867, 430, '잘 모르지만 배운 전공과목은 어렵다', WHITE, BLACK)
        CharactorSelect((7 * (padWidth // 8)) * 0.867, 460, '그는 대학을 성공적으로 졸업할 수 있을까..?', WHITE, BLACK)
        CharactorSelect((7 * (padWidth // 8)) * 0.867, 490, '인생은 한순간..그의 유일한 취미는 로또구매', WHITE, BLACK)
        CharactorSelect((7 * (padWidth // 8)) * 0.867, 550, "<승리조건>", WHITE, BLACK)
        CharactorSelect((7 * (padWidth // 8)) * 0.867, 580, "로또당첨 / 졸업요건(8번방문)만족시 승리!", WHITE, BLACK)
